...
class MainWindow(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        self.legacy_artificial_channels = artificial_channels
        # ------------------------------------------------------------
        ...
        self.setup_artificial_channel_info(artificial_channels=artificial_channels)
    ...
    def setup_artificial_channel_info(self, artificial_channels=None):
        artificial_kwargs = self._legacy_artificial_channel_kwargs(
            artificial_channels
        )
    ...
    def _legacy_artificial_channel_kwargs(self, artificial_channels):
        """
        Accept the older start_zmeter.py artificial_channels dictionary.

        The previous default was usually:
            A = nidaq_0_AO0 + nidaq_0_AO1
            B = nidaq_0_AO0 - nidaq_0_AO1
        The newer ArtificialChannelLogic represents the same affine transform
        through two artificial names, two original names, and three reference
        coordinate pairs.
        """
        if not artificial_channels:
            return {}

        artificial_names = list(artificial_channels.keys())
        if len(artificial_names) < 2:
            print(
                "[MainWindow] Ignoring legacy artificial_channels: expected two artificial channels."
            )
            return {}

        original_names = []
        for equation in artificial_channels.values():
            for token in re.findall(r"[A-Za-z_]\w*", equation):
                if token not in artificial_channels and token not in original_names:
                    original_names.append(token)

        if len(original_names) < 2:
            print(
                "[MainWindow] Could not infer original channels from legacy artificial_channels."
            )
            return {
                "artificial_channel_x_name": artificial_names[0],
                "artificial_channel_y_name": artificial_names[1],
            }

        return {
            "original_channel_x_name": original_names[0],
            "original_channel_y_name": original_names[1],
            "artificial_channel_x_name": artificial_names[0],
            "artificial_channel_y_name": artificial_names[1],
        }
