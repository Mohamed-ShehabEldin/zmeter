...
class Montana2Logic(QtCore.QThread):
    ...
    sig_stage1_temperature = QtCore.pyqtSignal(object)
    ...
    def _ensure_connected(self):
        if self.hardware is None or not self.is_connected:
            self.sig_status.emit("Montana is not connected.")
            return False
        return True
    ...
    def run(self):
        ...
        try:
            ...
            self.sig_status.emit(f"Montana job '{self.job}' failed: {exc}")
        finally:
            self.job = ""
