...
Features:
  ...
  • Crosshair cursor, zoom/pan (pyqtgraph)
  • Preserves user zoom/pan across acquisitions
...
class ethernet_winspec(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        uic.loadUi("winspec_remote_workshop/remote_winspec.ui", self)
        ...
        # Track whether we've plotted at least once (for auto-range logic)
    ...
    def _on_acquire_done(self, result):
        ...
        # Update data – curve redraws within the current view range
        ...
        # Only auto-range on the very first plot; after that keep user's zoom
        if self._first_plot:
